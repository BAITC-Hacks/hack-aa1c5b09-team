...
import { emptyCard } from '../api/types';
...
export function Editor() {
  ...
  if (query.data.status === 'selected') return <Navigate to={`/requests/${id}`} replace />;
...
function EditorContent({ request }: { request: NeedRequest }) {
  ...
  const fieldCount = Object.keys(emptyCard()).length;
...
const fields: { key: keyof NeedCard; label: string; placeholder: string; required?: boolean; multiline?: boolean }[] = [
  ...
  { key: 'requirements', label: 'Ограничения и пожелания', placeholder: 'Технологии, доступы и другие границы', multiline: true },
  { key: 'materials', label: 'Данные и материалы', placeholder: 'Доступные данные, примеры или источники', multiline: true },
  { key: 'successCriteria', label: 'Критерии успеха', placeholder: 'Измеримые признаки принятия результата — каждый с новой строки', multiline: true },
  { key: 'targetUsers', label: 'Пользователи решения', placeholder: 'Для кого создаётся решение', multiline: true },
  { key: 'businessContact', label: 'Контакт со стороны бизнеса', placeholder: 'Контакт, который можно публиковать в карточке' },
  { key: 'consultationFormat', label: 'Формат консультаций', placeholder: 'Как и когда можно обсудить задачу', multiline: true },
  { key: 'feedbackProcedure', label: 'Порядок обратной связи', placeholder: 'Кто, как и в какой срок проверяет результат', multiline: true },
];
function Review({ request, onHistory }: { request: NeedRequest; onHistory: () => void }) {
  ...
  const [card, setCard] = useState<NeedCard>(() => ({ ...emptyCard(), ...request.card, ...stored<Partial<NeedCard>>(storageKey, {}) }));
  const [saved, setSaved] = useState(false);
  const [editRevision, setEditRevision] = useState(request.revision);
  const dirty = Object.keys(emptyCard()).some(key => (card[key as keyof NeedCard] || '').trim() !== (request.card[key as keyof NeedCard] || '').trim());
  useEffect(() => { if (!dirty) setEditRevision(request.revision); }, [dirty, request.revision]);
  useEffect(() => { stash(storageKey, card); }, [card, storageKey]);
  const save = useMutation({ mutationFn: () => api.updateDraft(request.id, card, editRevision), onSuccess: result => { client.setQueryData(['request', request.id], result); void client.invalidateQueries({ queryKey: ['requests'] }); setSaved(true); setEditRevision(result.revision); forget(storageKey); } });
  const publish = useMutation({ mutationKey: ['publish', request.id], mutationFn: async () => { await api.updateDraft(request.id, card, editRevision); return api.publish(request.id); }, onSuccess: result => { forget(storageKey); client.setQueryData(['request', request.id], result); void client.invalidateQueries({ queryKey: ['requests'] }); navigate(`/requests/${request.id}`, { replace: true, state: { published: true } }); } });
  function submit(event: FormEvent) { event.preventDefault(); if (!save.isPending && !publish.isPending) { if (request.status === 'published') save.mutate(); else publish.mutate(); } }
  return <div className={s.reviewGrid}><form className={s.reviewForm} onSubmit={submit}><div className={s.panelTitle}><FileCheck2 size={20} /><h2>Карточка потребности</h2><span>* Обязательные поля</span></div><div className={s.fieldsGrid}>{fields.map(field => <label key={field.key} className={field.multiline || field.key === 'title' ? s.fieldFull : ''}>{field.label}{field.required && <span className={s.required}> *</span>}{field.multiline ? <textarea rows={3} required={field.required} maxLength={4000} placeholder={field.placeholder} value={card[field.key]} onChange={event => { setSaved(false); setCard({ ...card, [field.key]: event.target.value }); }} /> : <input required={field.required} maxLength={field.key === 'title' ? 120 : 250} placeholder={field.placeholder} value={card[field.key]} onChange={event => { setSaved(false); setCard({ ...card, [field.key]: event.target.value }); }} />}</label>)}</div><ErrorNotice error={save.error || publish.error} />{saved && <p className={s.successText} role="status"><CheckCircle2 size={16} />{request.status === 'published' ? 'Изменения сохранены' : 'Черновик сохранён'}</p>}<div className={s.reviewActions}><Button type="button" variant="secondary" busy={save.isPending} disabled={publish.isPending} onClick={() => save.mutate()}>{request.status === 'published' ? 'Сохранить изменения' : 'Сохранить черновик'}</Button>{request.status === 'draft' && <Button type="submit" busy={publish.isPending} disabled={save.isPending || !card.title.trim() || !card.description.trim() || !card.outcome.trim()}>Опубликовать заявку<ArrowRight size={17} /></Button>}</div></form><aside className={s.reviewAside}><ReadinessPanel request={request} editable unsaved={request.hasUnsavedCard || dirty || save.isPending || publish.isPending} /><div className={s.reviewAdvice}><span className={s.largeSpark}><Sparkles size={23} /></span><h3>Хорошая задача —<br />половина решения</h3><p>Чем понятнее ожидаемый результат, тем точнее будут предложения.</p><ul><li><Check size={15} />Проверьте важные детали</li><li><Check size={15} />Укажите комфортные сроки</li><li><Check size={15} />Оставьте место для идей</li></ul><p className={s.reviewWarning}>Карточку можно дополнять до выбора решения. После изменения сведений соответствующий показатель нужно подтвердить заново.</p></div>{request.status === 'draft' && <Button variant="ghost" onClick={onHistory}><MessageSquare size={16} />Посмотреть диалог с ИИ</Button>}</aside></div>;
}
