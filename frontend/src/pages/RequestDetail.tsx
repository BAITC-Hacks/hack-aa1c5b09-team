...
import { ArrowLeft, ArrowRight, BadgeCheck, Check, CheckCircle2, Clock3, FileText, MessageSquare, Sparkles, Star, Wallet } from 'lucide-react';
...
function DetailContent({ request }: { request: NeedRequest }) {
  ...
  const select = useMutation({ mutationFn: (offerId: string) => api.selectOffer(request.id, offerId), onSuccess: updated => { client.setQueryData(['request', request.id], updated); void client.invalidateQueries({ queryKey: ['requests'] }); setConfirm(null); setParams({ offer: updated.selectedOfferId! }, { replace: true }); } });
  return <>
    <Link to="/" className={s.backLink}><ArrowLeft size={16} />Мои заявки</Link>
    {notice && <div className={s.successBanner} role="status"><CheckCircle2 size={21} /><div><strong>Ваша заявка опубликована!</strong><span>{isDemo ? 'Для знакомства с продуктом мы добавили демонстрационные предложения.' : 'Теперь исполнители смогут предложить свои решения.'}</span></div><button onClick={() => setNoticeDismissed(true)} aria-label="Скрыть уведомление">×</button></div>}
    ...
    <div className={s.sectionHeading}><div><h2>Предложения <span>{offers.data?.length ?? request.offerCount}</span></h2><p>{request.status === 'selected' ? 'Вы нашли своё решение.' : 'Разные подходы к одной задаче. Выбор за вами.'}</p></div>{isDemo && <span className={s.smallDemo}><Sparkles size={13} />Демонстрационные отклики</span>}</div>
    <div className={`${s.offersLayout} ${selectedChat ? s.offersWithChat : ''}`}><div className={s.offersList}>{offers.isPending ? <Loading text="Загружаем предложения…" /> : offers.error ? <ErrorNotice error={offers.error} retry={() => void offers.refetch()} /> : !offers.data.length ? <EmptyState icon={<MessageSquare size={26} />} title="Ваша задача ждёт своего человека" text="Новые предложения появятся здесь. Можно вернуться чуть позже." /> : offers.data.map(offer => <article className={`${s.offerCard} ${request.selectedOfferId === offer.id ? s.offerSelected : ''}`} key={offer.id}><div className={s.offerTop}><Avatar name={offer.name} color={offer.color} size="large" /><div><h3>{offer.name}{request.selectedOfferId === offer.id && <BadgeCheck size={17} />}</h3><span>{offer.specialty}</span></div><div className={s.rating}><Star size={13} fill="currentColor" /><strong>{offer.rating.toFixed(1)}</strong><span>({offer.reviews})</span></div></div><p className={s.offerDescription}>{offer.description}</p><div className={s.offerTerms}><span><Wallet size={16} /><strong>{offer.price}</strong></span><span><Clock3 size={16} />{offer.duration}</span></div><div className={s.offerActions}><Button variant="secondary" onClick={() => setParams({ offer: offer.id }, { replace: true })}><MessageSquare size={16} />Обсудить</Button>{request.status === 'published' && <Button onClick={() => { select.reset(); setConfirm(offer); }}>Выбрать исполнителя<ArrowRight size={16} /></Button>}{request.selectedOfferId === offer.id && <span className={s.chosenLabel}><Check size={15} />Ваш выбор</span>}</div></article>)}</div>{selectedChat && <ChatPanel key={selectedChat.id} request={request} offer={selectedChat} onClose={() => setParams({}, { replace: true })} />}</div>
