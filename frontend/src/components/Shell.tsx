  ...
  return <div className={s.app}>
    ...
    <aside className={`${s.sidebar} ${menu ? s.sidebarOpen : ''}`}>
      ...
      <div className={s.sidebarBottom}>
        ...
        <div className={s.profile}><Link to="/profile" className={s.profileLink} onClick={() => setMenu(false)} aria-label="Открыть мой профиль"><Avatar name={user.name} /><span><strong>{user.name}</strong><small>Мой профиль</small></span></Link><button className={s.iconButton} aria-label="Выйти из аккаунта" disabled={logout.isPending} onClick={() => logout.mutate()}><LogOut size={18} /></button></div>
        <ErrorNotice error={logout.error} />
      </div>
    </aside>
    <div className={s.mainWrap}>
      <header className={s.topbar}><div className={s.breadcrumb}><button className={`${s.iconButton} ${s.mobileMenu}`} aria-label="Открыть меню" onClick={() => setMenu(true)}><Menu size={22} /></button><span className={s.breadcrumbRoot}>Личное пространство</span><ChevronRight size={14} />{location.pathname === '/profile' ? <span>Мой профиль</span> : <><Link to={section.to}>{section.label}</Link>{(location.pathname.startsWith('/requests/') || location.pathname.startsWith('/catalog/')) && <><ChevronRight size={14} /><span>{location.pathname.endsWith('/new') ? 'Новая заявка' : 'Потребность'}</span></>}</>}</div><div className={s.topbarRight}><button className={s.helpButton} onClick={() => setHelp(true)}><CircleHelp size={17} /><span>Помощь</span></button><span className={s.topbarDivider} /><Link to="/profile" aria-label="Открыть мой профиль"><Avatar name={user.name} size="small" /></Link></div></header>
