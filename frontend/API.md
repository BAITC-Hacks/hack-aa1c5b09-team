...
Актуальный HTTP-адаптер `src/api/http.ts` подключён к Java API. Источники действующего контракта: [REST API](../backend/docs/api/README.md), [рейтинг](../backend/docs/api/READINESS.md). Он использует `/needs`, `/me/needs`, `/me/proposals`, сессию и CSRF; преобразует ответы сервера во внутренние модели `src/api/types.ts`.

Ниже сохранён первоначальный проект интерфейса API для справки. Его маршруты `/catalog`, `/offers/mine` и основные операции `/requests` не являются действующими маршрутами Java API; исключение — переписка `/requests/:id/offers/:offerId/messages`. Уточняющий диалог сейчас выполняется в браузере, без серверного ИИ.
...
| `PATCH /requests/:id` | `{ card: NeedCard, revision? }` | Обновлённый `NeedRequest` |
| `PUT /requests/:id/readiness-confirmations` | `{ revision, confirmedCriteria }` | `NeedRequest` с пересчитанным рейтингом |
...
Для рейтинга в NeedCard добавлены строки `materials`, `successCriteria`, `targetUsers`, `businessContact`, `consultationFormat`, `feedbackProcedure`. NeedRequest дополнен `revision` и `readiness` в формате Java API. Если старый HTTP-сервер не возвращает рейтинг, UI не выдаёт его отсутствие за нулевой балл: блок рейтинга не показывается.

Демонстрационный редактор позволяет дополнять опубликованные задачи до выбора решения. Изменённые показатели теряют подтверждение; после сохранения владелец подтверждает их заново. Нулевой рейтинг не блокирует публикацию или предложения. Итоговое число нельзя принимать от клиента в настоящем API.
...
- Правки разрешены для `draft` с `readyForReview: true` и для `published`, до выбора решения. Название: до 120 символов; многострочный текст: до 4000; остальные поля: до 250. Пробелы обрезаются по краям. В запрос обновления передаётся `revision`; устаревшая версия возвращает 409.
- Публикация требует `title`, `description`, `outcome`. Дополнение опубликованной задачи сохраняет её статус и отклики. Повторная публикация в mock возвращает ту же заявку без дублирования предложений.
