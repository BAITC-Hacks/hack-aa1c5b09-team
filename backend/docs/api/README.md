...
| PUT /api/needs/{id} | Владелец черновика/опубликованной задачи | 200, Need |
| PUT /api/needs/{id}/readiness-confirmations | Владелец, до выбора решения | 200, Need с пересчитанным рейтингом |
...
Для всех операций записи требуется CSRF. Повторная публикация, принятие после выбора, редактирование после выбора решения и предложение к неопубликованной/закрытой карточке возвращают 409. Один автор может отправить одно предложение на карточку.
...
**Создание и замена карточки используют один формат.** PUT заменяет все редактируемые поля; это не частичный PATCH. Поле card совпадает по структуре с будущим suggestedCard ИИ. При изменении передавайте `revision` из последнего ответа Need. Для опубликованной задачи revision обязателен; для старых клиентов черновиков допускается отсутствие. Несовпадение версии возвращает 409 STALE_NEED. Обязательные поля опубликованной карточки нельзя очистить.
...
| materials | До 5000 символов: доступные данные, примеры, источники | Необязательно |
| targetUsers | До 2000: пользователи решения | Необязательно |
| businessContact | До 500: публичный контакт со стороны бизнеса | Необязательно |
| consultationFormat | До 2000: формат консультаций | Необязательно |
| feedbackProcedure | До 2000: порядок обратной связи | Необязательно |
...
Пример основных полей Need (дополнительно ответ всегда содержит `revision` и `readiness`; их формат описан в [контракте рейтинга](READINESS.md)):
