...
| PUT /api/auth/me | Вошедший пользователь + CSRF | 200, обновлённый User |
| GET /api/providers/{id} | Любой | 200, публичный профиль без email |
...
**Аккаунт содержит идентификатор, email, отображаемое имя и поля профиля.**
...
Профиль обновляется через PUT /api/auth/me. Тело содержит displayName (2–100 символов), specialty и location (до 120 символов), bio (до 2000 символов). Пустые необязательные поля сохраняются как null. GET /api/providers/{id} возвращает id, displayName, specialty, location и bio; email в публичный ответ не входит.
